<?php
	...
	if( have_posts() ) :
                        ...
                        <?php if( $format == "image" || empty($format) ) :
                                if( has_post_thumbnail() ) :?>
                                    <div class="entry-thumb">
                                        <a href="<?php the_permalink();?>" title="<?php printf(esc_attr__('Permalink to %s','agencies'),the_title_attribute('echo=0'));?>"><?php the_post_thumbnail("full");?></a>
                        ...
                        <?php if( $post_style == "entry-date-left"): ?>
                                ...
                                <div class="entry-details">
                                	...
                                	<div class="entry-title">
                                    	<h4><a href="<?php the_permalink();?>" title="<?php printf( esc_attr__('Permalink to %s','agencies'), the_title_attribute('echo=0'));?>"><?php the_title(); ?></a></h4>
                                    </div>
                                    
                                   <?php if( !empty($allow_excerpt) && !empty($excerpt) ):?>
                                    		<div class="entry-body"><?php echo agencies_excerpt($excerpt);?></div>
                                   <?php endif;?>
                                   ...
                                   <div class="entry-meta-data">
                                   		...
                                   		<p class="author <?php echo esc_attr( $show_author_meta );?>">
                                            ...
                                            <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                                            	title="<?php esc_attr_e('View all posts by ', 'agencies'); echo get_the_author();?>"><?php echo get_the_author();?></a>
                                        </p>
                                   ...
                                   <?php if( !empty($allow_read_more) && !empty($read_more) ):
								   			if( shortcode_exists( 'dt_sc_button' ) && agencies_is_plugin_active('js_composer/js_composer.php') ):
												$read_more = stripcslashes($read_more);
												$sc = str_replace("]",' link="url:'.$link.'"]',$read_more);
												$sc = do_shortcode($sc);
												echo !empty( $sc ) ? $sc : '';
											else:
												echo '<a href="'.get_permalink().'" title="'.get_the_title().'" class="dt-sc-button small filled dt-sc-readmore-link">'.esc_html__('Read More', 'agencies').'</a>';
											endif;
                        ...
                        <?php elseif( $post_style == "entry-date-author-left"):?>
                        		...
                        		<div class="entry-date-author">
                                    ...
                                    <div class="entry-author <?php echo esc_attr( $show_author_meta );?>">
                                    	...
                                    	<a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
                                        	title="<?php esc_attr_e('View all posts by ', 'agencies'); echo get_the_author();?>"><span><?php echo get_the_author();?></span></a>
                                    </div>
                        ...
                        <?php else: # Default Post Style ?>
                        		<div class="entry-details">
                                    ...
                                    <div class="entry-meta">
                                    	...
                                    	<div class="date <?php echo esc_attr($show_date_meta);?>"><?php esc_html_e('Posted on','agencies'); ?>  <?php echo get_the_date ( get_option('date_format') );?></div>
        ...
        <!-- **Pagination** -->
        <div class="pagination blog-pagination"><?php echo agencies_pagination();?></div><!-- **Pagination** -->
        <!-- Blog Template Ends --><?php

	else:?>
    	<h2><?php esc_html_e('Nothing Found.', 'agencies'); ?></h2>
        <p><?php esc_html_e('Apologies, but no results were found for the requested archive.', 'agencies'); ?></p><?php
	endif;?>